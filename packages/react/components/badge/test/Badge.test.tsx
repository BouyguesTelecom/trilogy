...
describe("Badge component", () => {
  ...
  test("should have a text content", () => {
    ...
    expect(screen.getByText("TEXTCONTENT").closest("div")).toHaveClass(
      "badge-and-text"
    );
    expect(screen.getByText("TEXTCONTENT").closest("div")).not.toHaveClass(
      "badge"
    );
  ...
  test("should have correct Text direction for Badge", () => {
    ...
    expect(
      screen.getByText(BadgeTextDirection.LEFT).nextElementSibling
    ).toBeNull();
    expect(
      screen.getByText(BadgeTextDirection.LEFT).previousElementSibling
    ).toHaveClass("text");

    expect(screen.getByText(BadgeTextDirection.RIGHT)).toHaveClass("badge");
    expect(
      screen.getByText(BadgeTextDirection.RIGHT).nextElementSibling
    ).toHaveClass("text");
    expect(
      screen.getByText(BadgeTextDirection.RIGHT).previousElementSibling
    ).toBeNull();
  });
  ...
  test("should have a correct color className", () => {
    getEnumNames(BadgeColor).forEach((element) => {
      ...
      expect(screen.getByText(element)).toHaveClass(
        has(`background-${getColorClassName(element)}`)
      );
    });
  });
