...
import { StatusState, TrilogyColor, getColorStyle } from '@/objects'
...
describe('Input component', () => {
  ...
  const status = [InputStatus.DEFAULT, InputStatus.ERROR, InputStatus.SUCCESS, InputStatus.WARNING]
  ...
  test('Should have search icon', () => {
    const { getByTestId } = render(<Input placeholder='Test' type={InputType.SEARCH} defaultValue='bonjour' />)
  ...
  test('Should have custom icon or status', () => {
    const { getByTestId } = render(<Input hasIcon status={InputStatus.SUCCESS} customIcon={IconName.INFOS_CIRCLE} />)
