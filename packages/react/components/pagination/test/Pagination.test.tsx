...
describe('Pagination component', () => {
  it('should render with default props', () => {
    const { getByRole, getByLabelText } = render(<Pagination length={10} />)
  ...
  it('should update the current page when a link is clicked', () => {
    ...
    const { getByLabelText } = render(<Pagination length={10} onClick={handleClick} />)
  ...
  it('should render the correct href when href prop is provided', () => {
    const { getByLabelText } = render(<Pagination length={10} href={(page) => `?page=${page}`} />)

    // Check if the first page link has the correct href attribute
    expect(getByLabelText('Aller à la page 1').getAttribute('href')).toBe('?page=1')
