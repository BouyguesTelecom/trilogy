...
describe("Box component", () => {
  ...
  test("Check box Table Container", () => {
    const { container } = render(
      <BoxTableContainer>Box Table</BoxTableContainer>
    );
  ...
  test("Should render box with markup A", () => {
    const fn = jest.fn();
    const { getByTestId } = render(
      <Box onClick={fn} testId="box" markup="A" to="test" />
    );
    const box = getByTestId("box");
  ...
  test("Should have classes content", () => {
    const { getByTestId } = render(
      <BoxContent testId="boxContent" background="SUCCESS" />
    );
  ...
  test("Should have classes footer", () => {
    const { getByTestId } = render(
      <BoxFooter testId="boxFooter" backgroundColor="SUCCESS" />
    );
  ...
  test("Should have classes header", () => {
    ...
    expect(boxFooter).toHaveClass(
      "box-header has-text-centered has-text-left has-text-right has-background-success"
    );
