 ...
 */
const BoxContent = ({ children, className, background, testId, ...others }: BoxContentProps): JSX.Element => {
  const { styled } = useTrilogyContext()

  const classes = hashClass(
    styled,
    clsx('box-content', background && has(getBackgroundClassName(background)), className),
  )
  return (
