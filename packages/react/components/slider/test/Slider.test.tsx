...
describe('Slider component', () => {
  ...
  describe('ref forwarding', () => {
    it('forwards the ref to the root <section> element', () => {
      ...
      expect(ref.current?.className).toContain('slider')
    })
  })

  describe('accessibility label fallback', () => {
    it('falls back to "Content slider" when ariaLabel is not provided', () => {
      const { container } = renderSlider()
      const section = container.querySelector('.slider') as HTMLElement
      expect(section).toHaveAttribute('aria-label', 'Content slider')
    })
  ...
  describe('displayName', () => {
    it('sets the Slider displayName to "Slider"', () => {
      expect(Slider.displayName).toBe('Slider')
    })
  })

  describe('SliderContext', () => {
    const ActiveIndexProbe = (): JSX.Element => {
      const { activeIndex } = React.useContext(SliderContext)
      return <div data-testid='probe'>{activeIndex}</div>
    }

    it('has a default activeIndex of 0 when consumed without a Provider', () => {
      const { getByTestId } = render(<ActiveIndexProbe />)
      expect(getByTestId('probe').textContent).toBe('0')
    })

    it('exposes the current active index to descendants and updates it on navigation', () => {
  ...
  describe('boundary: slidesPerView covers the whole set', () => {
    it('keeps the single dot active when next/prev are used and every slide already fits in one view', () => {
      const onSlideChange = jest.fn()
      const { container } = render(
        <Slider slidesPerView={6} onSlideChange={onSlideChange}>
          {Array.from({ length: 6 }).map((_, i) => (
            <SliderItem key={i}>
              <div>Slide {i + 1}</div>
            </SliderItem>
