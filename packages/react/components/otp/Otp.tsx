...
const Otp = ({
  ...
  const classes = hashClass(
    styled,
    clsx('otp-list', error && is('error'), activated && is('tertiary'), disabled && is('disabled'), className),
  )
  ...
  const inputOnChange = (e: React.ChangeEvent<HTMLInputElement>, idx: number) => {
    ...
    if (targetValueLength === 1) {
      const newValue = codeInput.substring(0, idx) + targetValue + codeInput.substring(idx + 1)
    ...
    <>
      ...
      {errorMessage && (
        <Text
          ...
          markup={TextMarkup.P}
          typo={(error && TypographyColor.TEXT_ERROR) || TypographyColor.TEXT_TERTIARY}
        >
          {errorMessage}
        </Text>
      )}
