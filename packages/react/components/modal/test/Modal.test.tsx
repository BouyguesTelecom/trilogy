...
describe("Modal component", () => {
  ...
  test("should Modal have correct className", () => {
    ...
    expect(
      screen.getByText("CLASSNAME").parentElement?.parentElement
    ).toHaveClass("className");
  });

  test("should Modal have correct contentClassNames", () => {
    render(
      <Modal contentClassNames={"contentClassNames"} content={"CLASSNAME"} />
    );

    expect(screen.getByText("CLASSNAME").parentElement).toHaveClass(
      "contentClassNames"
    );
  });
  ...
  test('should Modal ctaContent is "toto"', () => {
    render(
      <Modal ctaContent={"toto"} ctaOnClick={jest.fn()} content={"DEFAULT"} />
    );
  ...
  test("should Modal ctaOnClick call on ctaContent click", () => {
    ...
    render(
      <Modal ctaContent={"toto"} ctaOnClick={ctaOnClick} content={"DEFAULT"} />
    );
  ...
  test("should a click on ctaCancel Modal call ctaCancelOnClick function", () => {
    ...
    render(
      <Modal active ctaCancelOnClick={ctaCancelOnClick} content={"DEFAULT"} />
    );
  ...
  test("should Modal triggerMarkup have a correct markup", () => {
    getEnumNames(ModalMarkup).forEach((element) => {
      render(
        <Modal
          triggerMarkup={element}
          triggerContent={element}
  ...
  test("should Modal is active", () => {
    ...
    expect(screen.getByText("ACTIVE").parentElement?.parentElement).toHaveClass(
      is("active")
    );
  });
  ...
  test("should Modal is not active", () => {
    ...
    expect(
      screen.getByText("DEFAULT").parentElement?.parentElement
    ).not.toHaveClass(is("active"));
    expect(
      screen.getByText("ACTIVE").parentElement?.parentElement
    ).not.toHaveClass(is("active"));
  });
  ...
  test("should Modal is panel", () => {
    ...
    expect(screen.getByText("PANNEL").parentElement?.parentElement).toHaveClass(
      is("panel")
    );
  });
  ...
  test("should Modal is not panel", () => {
    ...
    expect(
      screen.getByText("DEFAULT").parentElement?.parentElement
    ).not.toHaveClass(is("panel"));
    expect(
      screen.getByText("PANNEL").parentElement?.parentElement
    ).not.toHaveClass(is("panel"));
  });
  ...
  test("should a click on closeIcon Modal call onClose function", () => {
    ...
    render(
      <Modal active closeIcon onClose={closeCallBack} content={"DEFAULT"} />
    );
