...
describe('SELECT OPTION WEB', () => {
  ...
  it('should have icon', () => {
    const { getByTestId } = render(
      <Select {...props}>
        {[...Array(5)].map((item, i) => {
          const value = `opt_${i}`
          return (
            <SelectOption key={i} id={value} value={value} label={value} testId={value} iconName={IconName.INFOS_CIRCLE}>
              {`option ${i}`}
            </SelectOption>
          )
        })}
    ...
    const option = getByTestId(`opt_2`)
    expect(option.firstElementChild?.firstChild?.firstChild).toHaveClass(IconName.INFOS_CIRCLE)
  })
})
