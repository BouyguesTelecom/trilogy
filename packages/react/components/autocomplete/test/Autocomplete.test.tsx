...
describe('AutoComplete', () => {
  ...
  it('selects an item from the menu', () => {
    ...
    expect(handleItemSelected).toHaveBeenCalledWith({ value: testItems[1], index: 1 })
  ...
  it('should have placeholder with default value', () => {
    render(<AutoComplete data={testItems} defaultValue='placeholder test' />)
    const input = screen.getByRole('textbox')
    expect(input).toHaveAttribute('placeholder', 'placeholder test')
  })

  it('should have input value, name', () => {
    render(<AutoComplete name='nameTest' data={testItems} inputValue='value test' />)
    const input = screen.getByRole('textbox')
  ...
  it('should be disabled', () => {
    ...
    const input = screen.getByRole('textbox')
    expect(input.parentNode).toHaveClass('is-disabled')
  ...
  it('should execute keypress down event', () => {
    ...
    const input = screen.getByRole('textbox')
    fireEvent.focus(input)
    fireEvent.keyUp(input, { key: 'arrow down', code: 'arrow down', keyCode: 40 })
    const items = screen.getAllByRole('listitem')
