...
describe('Progress', () => {
  it('renders correctly with percent value', () => {
    const { getByTestId } = render(<Progress percent={50} data-testid='progress-bar' />)
    const progressBar = getByTestId('progress-bar')
    expect(progressBar).toHaveAttribute('value', '50')
  })

  it('renders correctly with maxPercent value', () => {
    const { getByTestId } = render(<Progress percent={25} maxPercent={50} data-testid='progress-bar' />)
    const progressBar = getByTestId('progress-bar')
    expect(progressBar).toHaveAttribute('max', '50')
  })

  it('renders correctly with unique legend', () => {
    const { getByText } = render(<Progress percent={50} uniqueLegend='50%' data-testid='progress-bar' />)
...
describe('ProgressRadial', () => {
  it('renders without crashing', () => {
    const { getByTestId } = render(
      <ProgressRadial percent={50} label='Test Label' description='Test Description' data-testid='progress-radial' />,
    )
    expect(getByTestId('progress-radial')).toBeInTheDocument()
  })

  it('renders label and description', () => {
    const { getByText } = render(<ProgressRadial percent={50} label='Test Label' description='Test Description' />)
