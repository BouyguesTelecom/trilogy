import { Calendar, ChangeEventCalendar } from '@/components/calendar/index.native'
  ...
  (
      ...
      (text: string) => {
        ...
        if (numbersOnly.length >= 5 && onChange) {
          ...
          if (dayNum >= 1 && dayNum <= 31 && monthNum >= 1 && monthNum <= 12 && yearNum >= 1000) {
            const formattedDate = `${yearNum}-${monthNum.toString().padStart(2, '0')}-${dayNum
              .toString()
              .padStart(2, '0')}`
            onChange(formattedDate)
          }
    ...
    const styles = StyleSheet.create({
      ...
      input: {
        flex: 1,
        fontSize: 16,
        color: disabled ? getColorStyle(TrilogyColor.DISABLED) : getColorStyle(TrilogyColor.MAIN),
      ...
      <View ref={ref} style={styles.container} testID={testId} {...others}>
        {label && (
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            {required && <Text style={{ color: getColorStyle(TrilogyColor.ERROR), marginLeft: 4 }}>*</Text>}
          </View>
        )}
        ...
        <View style={styles.inputContainer}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <Icon name='tri-calendar' size={IconSize.SMALL} color={TrilogyColor.MAIN} />
          </TouchableOpacity>
        </View>
        ...
        <Modal visible={isCalendarVisible} transparent={true} animationType='fade' onRequestClose={handleCloseCalendar}>
          <View style={styles.modalOverlay}>
            <TouchableOpacity style={StyleSheet.absoluteFillObject} activeOpacity={1} onPress={handleCloseCalendar} />
